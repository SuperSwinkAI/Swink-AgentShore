...
const UV_VENV_TIMEOUT: Duration = Duration::from_secs(10 * 60);
const WHEEL_INSTALL_TIMEOUT: Duration = Duration::from_secs(45 * 60);
const SIDECAR_IMPORT_TIMEOUT: Duration = Duration::from_secs(2 * 60);
const BD_PROVISION_TIMEOUT: Duration = Duration::from_secs(10 * 60);
const TIMELAPSE_TIMEOUT: Duration = Duration::from_secs(45 * 60);
const CLI_TIMEOUT: Duration = Duration::from_secs(20 * 60);
...
type ProvisionResult<T> = Result<T, ProvisionError>;
...
impl Logger {
    fn open(step_name: &str) -> ProvisionResult<Self> {
...
impl ParsedArgs {
    fn parse(args: Vec<OsString>) -> ProvisionResult<Self> {
    ...
    fn required_path(&self, flag: &str) -> ProvisionResult<PathBuf> {
        self.pairs
            .iter()
...
fn replace_venv_with_rollback<F>(venv: &Path, logger: &Logger, install: F) -> ProvisionResult<()>
where
...
fn stop_recorded_runtime_processes(logger: &Logger) -> ProvisionResult<()> {
...
fn grant_read_execute(path: &Path, logger: &Logger) -> ProvisionResult<()> {
    run_icacls(
        path,
...
fn grant_modify(path: &Path, logger: &Logger) -> ProvisionResult<()> {
    run_icacls(
        path,
...
fn cleanup_legacy_layout(logger: &Logger) {
...
fn validate_file(path: &Path, label: &str) -> ProvisionResult<()> {
    if path.is_file() {
        Ok(())
    } else {
...
fn safe_log_name(step_name: &str) -> String {
