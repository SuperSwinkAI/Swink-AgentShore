...
# ---------------------------------------------------------------------------
# PlayOutcome.failed() produces alignment_delta=0.0 (not None)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# RewardSignals.alignment_delta is now float | None
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Reward: None alignment_delta + SEED_PROJECT → small bonus
# ---------------------------------------------------------------------------
...
def test_reward_none_alignment_seed_project_gets_bonus() -> None:
    ...
    # SEED_PROJECT with no beads graph should get the small 0.05 seeding bonus
...
def test_reward_none_alignment_default_signals_returns_zero() -> None:
    # Default RewardSignals has alignment_delta=None and play_type=None
...
# ---------------------------------------------------------------------------
# Reward: float alignment_delta still works normally
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Epic closure ratio features in slots 8-11
# ---------------------------------------------------------------------------
...
def test_epic_top3_closure_ratios_sorted_by_total_tasks_desc() -> None:
    ...
    state = _make_state(graph=graph)
    obs = encode_observation(state, _null_ctx())
    # largest (total_tasks=20) → slot TOP0 → 0.9
    assert obs[_S_EPIC_TOP0] == pytest.approx(0.9, abs=1e-5)
    # medium (total_tasks=10) → slot TOP1 → 0.3
    assert obs[_S_EPIC_TOP1] == pytest.approx(0.3, abs=1e-5)
    # small (total_tasks=2) → slot TOP2 → 0.5
...
def test_epic_top3_padded_when_fewer_than_3_epics() -> None:
    ...
    # Unfilled epic slots should remain 0.0
...
def test_epic_closure_ratio_clamped_to_0_1() -> None:
    # A closure_ratio > 1.0 would be a data error but should clamp
    epics = [
...
# ---------------------------------------------------------------------------
# Cluster slots 0-7 are all 0.0 in v0.10.0 (goal_clusters removed)
# ---------------------------------------------------------------------------


def test_cluster_slots_0_to_7_are_zero_in_v0_10() -> None:
    # goal_clusters removed; legacy cluster slots must be 0.0.
    state = _make_state()
    obs = encode_observation(state, _null_ctx())
    assert obs[0:8].sum() == pytest.approx(0.0, abs=1e-5)
    # Epic slots should also be 0 (no graph)
    assert obs[_S_EPIC_GLOBAL_RATIO] == pytest.approx(0.0, abs=1e-5)
...
def test_dependency_slots_and_retired_cluster_slots() -> None:
    ...
    assert obs[1] == pytest.approx(0.5, abs=1e-5)  # ready_task_ratio
