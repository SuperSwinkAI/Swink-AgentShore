...
- Instantiate CLI agents (`claude_code`, `codex`, `grok`, `antigravity`, `swink_coding`) as subprocess-backed handles. All supported agents are CLI-harnessed; raw HTTP/API agents are not supported. This holds for local/alternate models too: `swink_coding` reaches Ollama/vLLM/OpenAI-compatible endpoints, but AgentShore only ever talks to the `swink-coding` CLI — swink-coding IS the CLI harness in front of those raw HTTP endpoints, so the no-API-agents policy is unchanged.
...
| `codex` | CLI subprocess | Codex CLI. Tiers: gpt-5.6-luna / gpt-5.6-terra / gpt-5.6-sol with reasoning effort low/medium/high via `-c model_reasoning_effort` (enum minimal/low/medium/high/xhigh). |
...
| `swink_coding` | CLI subprocess | SuperSwink-Coding CLI (binary `swink-coding`) — the local/alternate-model harness (#317): Ollama, Mistral, vLLM/LM Studio/llama.cpp, any OpenAI-compatible endpoint. NDJSON on stdout (`session.started` → deltas → terminal `result` with Anthropic-style usage keys); `session.started` is written **before any model call**, so the first-byte watchdog is satisfied by design even for cold local models. Non-interactive via `--yolo`. **Tier-opaque model mapping: the tier alias IS the model** — `--model` accepts only `small\|medium\|large`, and the tier→concrete-model map lives in swink-coding's own `~/.swink-coding/config.toml` (a catalog-picked concrete model dispatches as `--model <tier> --tier-map <tier>=<model>`, still resolved by swink-coding). No effort flag (`reasoning_effort` rejected at config-parse). Local tiers report "no credential required" to the auth probe and bill $0. |

Capabilities are declared statically per type in `capabilities.py` (`can_implement`, `can_review`, `can_test`, `can_create_pr`, `can_merge`, `can_create_issues`, plus `max_context` and per-token cost). Merge and issue creation are deliberately available to every type - they are AgentShore-mediated GitHub/repository operations, so scheduler availability can never strand an approved PR behind a disabled or saturated provider. `swink_coding` declares the full capability set but a deliberately conservative `max_context` (32,768) — the band for typical local-model context windows, not the frontier-provider values the other types carry.
...
The flat model catalog (`model_catalog.py`) can expose additional opt-in models without making them defaults. Current examples: `claude-sonnet-5` and `claude-fable-5` for Claude, and `gpt-5.5`/`gpt-5.4`/`gpt-5.4-mini` for Codex (prior-generation models kept selectable alongside the gpt-5.6 default trio). `gpt-5.5-pro` and `gpt-5.3-codex-spark` are deliberately excluded from the curated list — both are gated (400s under ChatGPT-account auth, or Pro-tier-only) in a way that would permanently kill the agent via `INVALID_MODEL` for non-eligible accounts; API-key/Pro users still reach them through the live model fetch. `gpt-5.4-nano` was retired by OpenAI and removed from the catalog 2026-07-11. Antigravity exposes explicit Gemini 3 model IDs. Grok is the exception: it is hard-pinned to `grok-4.5` (the only model the Grok CLI accepts for coding; the retired `grok-build` name collapses to it), so the catalog exposes no grok alternatives — grok tiers vary only by reasoning effort.

The catalog itself is data, not code: it ships in `agentshore/data/models.yaml` and is overridable by a global touchpoint (`paths.GLOBAL_MODELS_PATH`) that wholesale-replaces a harness's list when present — same shape as `pricing.yaml`'s bundled + global-override pattern (`model_catalog.load_model_catalog`). This is the groundwork for a desktop "Refresh Models" action: a refresh writes to the global override file, and every consumer (`sidecar/agents.py:agents_catalog`, the CLI wizard) picks it up without a restart.

### Endpoint Delegation — the `api_base` Decision (#327)

AgentShore never configures model endpoints, for any agent type. **Decision (2026-07-23, closing #327): endpoint and concrete-model resolution for `swink_coding` is fully delegated to swink-coding's own config (`~/.swink-coding/config.toml`); AgentShore does not carry an endpoint field.** That inversion is the entire point of the tier abstraction — AgentShore dispatches an opaque `small|medium|large` alias (or a `--tier-map` override whose value swink-coding still resolves) and never sees a URL or provider-native model id. The historical `AgentConfig.api_base` field — a dormant hook that was parsed but consumed by nothing on the argv/dispatch path — has been removed from config entirely; there is nothing for it to do that would not duplicate, and eventually fight, swink-coding's own resolution. The rejected alternative (plumbing per-agent endpoint overrides through as generated swink-coding `--config` flags) would put AgentShore back in the business of knowing endpoints and split the source of truth across two files. If a user needs a different endpoint, they edit swink-coding's TOML; AgentShore is agnostic by design.

### Model Discovery

Discovering each harness's *currently* selectable models — as opposed to the hand-curated baseline above — splits into two mechanisms depending on what the CLI exposes, confirmed by directly probing each installed CLI:

- **Codex, Grok, Antigravity, swink-coding — free, deterministic, local (`agents/model_discovery.py`).** Each CLI has its own subcommand that enumerates current models with no API key and no token spend: `codex debug models` (structured JSON: slug/display_name/visibility), `grok models` (plain text, default marked), `agy models` (plain text), `swink-coding models --json` (JSON per-provider/endpoint rows — the always-selectable tier aliases first, then each reachable provider's concrete models). Output shape and default ordering aren't guaranteed stable across CLI versions, so parsing treats results as a set, not a positional diff. `discover_all()` probes all of them; each probe is independently timeout-bounded and tree-kills its subprocess on timeout (mirrors `agents/auth_probe.py`'s `Popen`-based process handling) so one hung CLI can't block the others.

- **Claude Code — no CLI surface exists (`agents/model_discovery_llm.py`).** No flag, no subcommand, no bundled manifest — its model IDs are baked into the compiled single-file binary with no way to distinguish current from years-deprecated ones (confirmed via `strings` on the installed binary). The only way to get a live answer is to dispatch a real Claude Code agent and ask it to look one up via web search, which **spends real API tokens** — a categorically different mechanism from the three above, and every call site must treat it as an explicit, user-opted-in action, never automatic.

  Measured during development (Haiku tier, real dispatches): a *successful* run costs **roughly $0.30–0.50** across ~10–13 turns of web search — not "a few cents." The dispatch uses `--safe-mode` (disables CLAUDE.md/plugins/hooks/MCP so the probe never inherits the caller's project config — a bare call from this repo picked up its CLAUDE.md and fired unrelated `SessionEnd` hooks) but deliberately **not** `--bare` (which forces `ANTHROPIC_API_KEY`-only auth, disabling OAuth — defeating the entire point of asking the CLI instead of hitting the raw provider API). It allows both `WebSearch` and `WebFetch` (allowing only `WebSearch` caused an observed run to burn 10 searches over 13 turns and $0.37 compensating for denied fetches instead of reading the canonical docs page directly), and passes `--max-budget-usd` as a real, confirmed-working hard spend ceiling (the CLI exits cleanly with `subtype: "error_max_budget_usd"` rather than hanging or overspending) — defaulted generously above the observed successful cost so a legitimate run isn't truncated into a wasted partial spend. `--output-format json` returns the provider-authoritative `total_cost_usd`, used directly for cost reporting instead of re-deriving via `pricing.yaml`.
...
`attempt_recovery()` transitions an `ERROR` agent back to IDLE when the breaker allows - but skips config-class errors (`auth`, `invalid_model`) that re-attempting can't fix. `TAKE_BREAK` is the play that drives recovery of agents whose error class is recoverable (`rate_limit`, `auth`, `transient_network`, `codex_rollout`, `unknown`, `no_op` — the single source of truth is `core/recovery_tracker.py:_RECOVERY_OVERRIDE_KIND`, drift-guarded equal to `state.RECOVERABLE_ERROR_CLASSES`). Timeouts carry a precise sub-class (wallclock / stream-idle / post-response) for sliced telemetry.

### Backend-auth recovery

A backend session can also expire *mid-run*, after a clean preflight. A live dispatch's stderr is sniffed for the backend-auth signatures (the same markers the probe uses, e.g. Codex's `failed to renew cache TTL`); on a hit the dispatch is aborted immediately with `ErrorClass.AUTH` instead of hanging to the full stream-idle timeout, and the handle lands in `ERROR`.

From there AUTH is handled exactly like a quota/rate-limit hold — take a break, then back to work — through the standard `TAKE_BREAK` machinery (`AUTH` deliberately reuses the `RATE_LIMIT_RECOVERY` override kind rather than introducing a parallel path). Because `attempt_recovery()` declines config-class errors, an agent whose token is genuinely dead fails its breaks and graduates to `END_AGENT` after `BREAK_RECOVERY_FAILURE_LIMIT` (2) consecutive failures — the *agent* retires, bounded and observable, while the agent *type* stays available: other same-type agents keep working and `INSTANTIATE_AGENT` can still spawn a fresh one.

Historical note: the original design latched the failed type into a session-wide grow-only suppression set (`last_auth_failed_types` → `auth_suppressed_agent_types`) that hard-masked every play targeting that type, including instantiation, for the rest of the session. That mechanism was removed in `aaaf0d1` — with no re-probe, decay, or recovery, one transient blip (a github.com wobble, a killed probe subprocess) disabled an entire harness until restart.
