...
- **Codex, Grok, Antigravity — free, deterministic, local (`agents/model_discovery.py`).** Each CLI has its own subcommand that enumerates current models with no API key and no token spend: `codex debug models` (structured JSON: slug/display_name/visibility), `grok models` (plain text, default marked), `agy models` (plain text). Output shape and default ordering aren't guaranteed stable across CLI versions, so parsing treats results as a set, not a positional diff. `discover_all()` probes all three; each probe is independently timeout-bounded and tree-kills its subprocess on timeout (mirrors `agents/auth_probe.py`'s `Popen`-based process handling) so one hung CLI can't block the others.
