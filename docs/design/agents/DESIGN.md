...
| `grok` | CLI subprocess | Grok CLI. `grok-build` is the only model; tiers differ solely by reasoning effort low/medium/high passed via `--effort` (enum low/medium/high/xhigh/max). |
