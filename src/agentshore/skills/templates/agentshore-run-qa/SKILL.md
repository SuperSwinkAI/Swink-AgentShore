...
- **Duplication & shape:** near-identical blocks a shared helper would collapse, shadow utilities reimplementing existing `utils/`/`helpers/`, over-abstraction (factory/wrapper for one use), logic bloat / meta-comments restating the next line, property drilling through many layers.
