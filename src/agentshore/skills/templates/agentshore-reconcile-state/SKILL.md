...
- **`zombie_subprocess`** — `ps` state `Z` AND ppid=1 (reparented) OR ppid matches a recent `cli_dispatch_start` whose play later emitted `agent_dispatch_timed_out`. Only classify if you can name the specific `play_id` and timeout event.
