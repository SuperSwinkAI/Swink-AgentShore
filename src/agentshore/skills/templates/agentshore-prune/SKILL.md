...
- **Worktree:** path matches `pickup-*` or `agentshore-*` under the AgentShore worktree root AND was created more than `worktree_min_age_hours` ago (default: 3 hours) AND one of the existing stale checks passes: its branch has no commits beyond merge-base with target, OR is the head of a closed/merged PR, OR has no open PR and no in-flight work claim.
...
**Pre-flight.** Read the per-play context file (path is in `$AGENTSHORE_PROJECT_PATH/.agentshore/context.json` under the key `context_file`; fall back to `context.json` itself) for `repo`, `owner`, `target_branch`, `session_id`, `active_worktree_paths` (a list of absolute paths that have live work claims — default to `[]` if absent), `young_worktree_paths` (absolute paths created less than `worktree_min_age_hours` ago — default to `[]` if absent), and `worktree_min_age_hours` (default `3`). Resolve `$TARGET` from `$ARGUMENTS` (override), context, or repo default. Resolve `$WORKTREE_ROOT` from `.agentshore/worktrees-root.txt`, then `$AGENTSHORE_WORKTREES_ROOT`, else `<parent-of-project>/agentshore-worktrees/<repo-name>/`.
...
**Worktree sweep.** From `git worktree list --porcelain`, for each worktree under `$WORKTREE_ROOT` matching `pickup-*`/`agentshore-*`: apply the age guard first — path in `young_worktree_paths` → keep (reason `created_less_than_3h`). If the worktree is not in `young_worktree_paths`, verify it was created more than `worktree_min_age_hours` ago using available filesystem metadata; if age cannot be confirmed, keep (reason `age_unknown`). Only worktrees that pass this age guard may continue to the stale checks. Then classify by branch — `closed_pr_branches` → orphan (reason `pr_closed:#N`); `open_pr_branches` → keep; path in `active_worktree_paths` → keep (reason `active_work_claim`); else `git rev-list --count $TARGET..<branch> == 0` → orphan (reason `no_commits_beyond_target`); else keep. Remove orphans with `git worktree remove --force <path>`; finish with `git worktree prune --verbose`. Failures (locked, perms) go to `worktrees_skipped` with the error.
...
**Closed-bead hygiene.** `bd` is reference-aware — it never prunes a closed issue still referenced by an open one. Preview with `bd prune --older-than 90d --dry-run`, then execute `bd prune --older-than 90d --force`. Record the reported count under `beads_pruned`. A prune error is reported as a `beads_pruned_error` string, not a play failure — this is a best-effort hygiene step, same as the other sweeps.
...
```json
{
  "success": true,
  "artifacts": [
    {"type": "prune_summary", "worktrees_pruned": 3, "branches_deleted_local": 4, "branches_deleted_remote": 2, "beads_closed_stale_linked": 12, "beads_pruned": 40}
  ],
